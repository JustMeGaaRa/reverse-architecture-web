...
import { useWorkspace, useWorkspaceCollaborative, useWorkspaceExplorer } from "../hooks";
import { WorkspaceExplorerProvider, YjsDocumentProvider, WorkspaceProvider, YjsWebrtcProviderProvider, YjsUndoManagerProvider, YjsIndexeddbPersistanceProvider } from "../components";
import { createWorkspaceConnection, create, createExplorerDocument, createWorkspaceDocument, createExplorerPersistance, createWorkspacePersistance, remove } from "../utils";
...
export const TestWorkspaceInfo: FC<{
    ...
    const { document } = useWorkspaceCollaborative();
...
export const TestWorkspaceCollection: FC = () => {
    const { document, setDocument, setPersistance } = useWorkspaceCollaborative();
...
export const TestWorkspace: FC = () => {
    const { undoManager } = useWorkspaceCollaborative();
...
export const TestWorkspaceLoader: FC<PropsWithChildren<{ workspaceId: string; }>> = ({ children, workspaceId }) => {
    ...
    const { document, setPersistance } = useWorkspaceCollaborative();
...
export const TestWorkspaceConnector: FC<PropsWithChildren<{ workspaceId: string; }>> = ({ children, workspaceId }) => {
    const [ isLoading, setIsLoading ] = useState(true);
    const { document, setConnection } = useWorkspaceCollaborative();
...
export const TestWorkspaceInitializer: FC<PropsWithChildren<{}>> = ({ children }) => {
    ...
    const { document, setUndoManager } = useWorkspaceCollaborative();
