...
export class Styles implements ISupportSnapshot<IStyles> {
    ...
    public constructor(private readonly propertiesMap: Y.Map<unknown>) {
        if (!propertiesMap.has("elements"))
            propertiesMap.set("elements", new Y.Array<ElementStyle>());
        if (!propertiesMap.has("relationships"))
            propertiesMap.set("relationships", new Y.Array<RelationshipStyle>());
    }
